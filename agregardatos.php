...
if($link === false){
	...
	// se tienen que gragar todos los campos de pabase de datos, si un cambo en la BD es declarado CHAR se debe colo car la variable 
	// entre apostrofes (Coillas simples) ejempo: '$nombre ', en caso de ser num, float, double, decimal o cualquier otro campo que sea 
	// numerico se debe colocar sin apostrofes, ejemplo: este = indice1 float NULL DEFAULT NULL.
	
	if($num3<=0){
			$sql="INSERT INTO `alumnos` 
			VALUES ('$rne ', '$nombre ', '$apellido ', '$email ', '$fnac ', '$pnace ', '$dnace ', '$municipio ', '$dir ', 
			'$barrio ', '$avenida ', '$calle ', '$numcasa ', '$telfijo ', '$vivienda ', '$nacionalidad ', '$habilitado ', 
			'$sexo ', '$escuela ', '$beca ', '$mdificil ', '$mfacil ', '$repeat ', '$cursorepetido ', '$mrepeat ', 
			'$mretrasada ', '$cviven ', '$p ', '$nhermanos ', '$nhermanas ', '$lugar ', '$numhermanos ', '$numhermanas ', 
			'$numhermanosadop ', '$numhermanasadop ', '$padre ', '$pid ', '$pedad ', '//$pfechan ', '$pcivil ', '$preligion ', 
			'$pdir ', '$ptrab ', '$plugar ', '$pcargo ', '$ptel ', '$ptelmovil ', '$ptiempo ', '$pemail ', '$pex ', '$madre ', 
			'$mid ', '$medad ', '$mcivil ', '$mreligion ', '$mdir ', '$mtrab ', '$mlugar ', '$mcargo ', '$mtel ', '$mtelmovil ', 
			'$mtiempo ', '$memail ', '$mexa ', '$encargado ', '$eid ', '$eedad ', '$ecivil ', '$ereligion ', '$edir ', '$etrab ', 
			'$elugar ', '$ecargo ', '$etel ', '$etelmovil ', '$etiempo ', '$eemail ', '$exa ', '$encargado1 '', '$encargadotel1 ',
			'$encargadotel1 ', '$encargado2 ', '$encargadotel2 ', '$encargado3 ', '$encargadotel3 ', '$problem ', '$saludespecifique ', 
			'$tsangre ', '$audio ', '$visual ', '$habla ', '$otro ', '$agresion ', '$ansiedad ', '$depresion ', '$bipolaridad ', 
			'$violencia ', '$hiperactividad ', '$deficit ', '$medicamentos ', '$despecifique ', '$antecedentes ', '$medies ', 
			'$medies1 ', '$medies2 ', '$snom ', '$singesta ', '$sdosis ', '$religion ', '$bautismo ', '$comunion ', '$confirmacion ', 
			'$grupojuvenil ', '$campamento ', '$magis ', '$respecifique ', '$rcantaespecifique ', '$rgrupoespecifique '
			);";

			if ($link->query($sql) === TRUE) {
				 echo"<script type=\"text/javascript\">alert(' Estudiante registrado'); window.location='';</script>";
			}
	}else{
		while($rows= $rs3->fetch_all()){
			$data[]=$rows;
		}
		echo"<script type=\"text/javascript\">alert(' Ya existe el estudidante'); window.location='';</script>";
		json_encode($data);
	}
